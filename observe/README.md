...
A troca entre o cluster `kubeadm-local` e o EKS é feita por uma única variável visível, `cluster` (rótulo "Cluster Source", tipo Custom, valores "Local" → `""` e "AWS" → `"AWS-"`). Como uma variável do tipo "Data source" só lista datasources de um plugin por vez, ela não dá para cobrir Prometheus/Loki/Tempo ao mesmo tempo; o modelo contorna isso com três variáveis "Data source" ocultas (`datasource`, `datasource_loki`, `datasource_tempo`, uma por `pluginId`) cujo `regex` referencia `${cluster}` (por exemplo `/^${cluster}prometheus$/`) para filtrar, a cada troca de `cluster`, o único datasource daquele tipo cujo nome bate com o prefixo escolhido. Cada painel referencia a variável oculta do seu tipo (`${datasource}`, `${datasource_loki}` ou `${datasource_tempo}`) em vez do datasource fixo. Esse esquema depende de uma convenção de nomes nos datasources cadastrados no Grafana: local sem prefixo (`prometheus`, `loki`, `tempo`) e AWS com o prefixo `AWS-` (`AWS-prometheus`, `AWS-loki`, `AWS-tempo`).
