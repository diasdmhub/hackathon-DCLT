...
A linha (row, recolhida por padrão) "Recursos dos Pods" traz 6 painéis stat, com CPU e memória sugeridas (p95) para cada um dos 3 serviços, calculados a partir do `kubelet-resource` (`container_cpu_usage_seconds_total`/`container_memory_working_set_bytes`, rotulados por `container`, que na solidarytech coincide com o nome do serviço: `ngo`/`donation`/`volunteer`). A query usa `avg by (container)` antes do `quantile_over_time` para obter o uso típico de **um** pod (não a soma da frota), o que mantém o número comparável a `requests`/`limits` do Deployment mesmo com o HPA (ver seção "Autoscaling (HPA)" no `CLAUDE.md`) variando a contagem de réplicas entre 1 e 4. Os thresholds (verde até o `request` atual, amarelo até o `limit`, vermelho acima) estão fixos com os valores hoje em `kube/0{40,50,60}-*/*.yaml`: se esses `resources.requests`/`resources.limits` mudarem, atualize os thresholds destes 6 painéis (mais os 6 espelhados na aba Local) para não ficarem desalinhados. O objetivo é dar o insumo (p95 de uso real) para reajustar manualmente `requests`/`limits` ao longo do tempo, já que o VPA foi descartado (causaria _drift_ contra o Flux, ver `doc/estrutura.md`).
