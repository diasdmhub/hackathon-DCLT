...
[`doc/grafana/dashboard-solidarytech.json`](/doc/grafana/dashboard-solidarytech.json) é um modelo de dashboard de visão geral (Grafana 13.1.1, importável via **Dashboards → New → Import**) com as métricas de negócio (Loki), RED por serviço, o mapa de serviços (Tempo/Prometheus) e a saúde dos pods da solidarytech (kube-state-metrics/Prometheus). A importação hoje é manual (colar o JSON ou fazer upload do arquivo); ainda não há sincronização automática a partir do Git.

A variável `datasource` (rótulo "Cluster Source", `pluginId: prometheus`, `regex: /prometheus/`) seleciona qual Prometheus consultar - o do `kubeadm-local` ou o do EKS (`terra/modules/prometheus`) - e é usada por todos os painéis Prometheus deste modelo (RED, hot path e saúde dos pods). Cadastre os dois datasources Prometheus no Grafana com nomes que casem com esse regex para poder alternar entre clusters.

Os painéis de infraestrutura que antes vinham do Zabbix (`CPU utilization`, `Memory utilization`, `Load average (1m avg)`, `Node Ready`) foram removidos deste modelo e reconstruídos, contra kube-state-metrics/node-exporter/Prometheus, em [`doc/grafana/dashboard-solidarytech-infra.json`](/doc/grafana/dashboard-solidarytech-infra.json) - um modelo separado, com a mesma variável `datasource`, dedicado a CPU/load/memória/disco por node e dados básicos do cluster (contagem de nodes e de pods em execução). A separação evita misturar infraestrutura de cluster (que só faz sentido olhar por cluster) com a visão de negócio/serviço da solidarytech.
