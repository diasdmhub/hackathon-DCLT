...
A troca entre o cluster `kubeadm-local` e o EKS é feita por uma única variável visível, `cluster` (rótulo "Cluster Source", tipo Custom, valores "Local" → `""` e "AWS" → `"AWS-"`). Como uma variável do tipo "Data source" só lista datasources de um plugin por vez, ela sozinha não cobriria Prometheus/Loki/Tempo ao mesmo tempo - e encadear uma segunda variável "Data source" cujo `regex` referenciasse `${cluster}` (testado numa primeira versão) não funciona no motor "scenes" do schema v2 usado por esses dashboards: o `regex` não é reavaliado quando `${cluster}` muda, deixando a variável sem opção válida e todo painel que dependia dela quebrando com "Datasource was not found". A solução foi eliminar essa variável intermediária: cada painel referencia o datasource diretamente no campo `datasource.name` da query, concatenando `${cluster}` com o nome do tipo - `${cluster}prometheus`, `${cluster}loki`, `${cluster}tempo` -, sem passar por nenhuma variável do tipo "Data source". Isso depende de duas coisas: (1) uma convenção de nomes nos datasources cadastrados no Grafana - local sem prefixo (`prometheus`, `loki`, `tempo`) e AWS com o prefixo `AWS-` (`AWS-prometheus`, `AWS-loki`, `AWS-tempo`); (2) o resolvedor de datasource do Grafana aceitar, nesse campo, um nome de datasource (não só um UID) - comportamento de fallback que existe há muitas versões, mas que não chegou a ser validado ao vivo contra essa instância antes de documentar aqui.

Os painéis de infraestrutura que antes vinham do Zabbix (`CPU utilization`, `Memory utilization`, `Load average (1m avg)`, `Node Ready`) foram removidos deste modelo e reconstruídos, contra kube-state-metrics/node-exporter/Prometheus, em [`doc/grafana/dashboard-solidarytech-infra.json`](/doc/grafana/dashboard-solidarytech-infra.json) - um modelo separado, com a mesma variável `cluster`, dedicado a CPU/load/memória/disco por node e dados básicos do cluster (contagem de nodes e de pods em execução). A separação evita misturar infraestrutura de cluster (que só faz sentido olhar por cluster) com a visão de negócio/serviço da solidarytech.
