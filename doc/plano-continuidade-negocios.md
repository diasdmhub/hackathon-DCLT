...
2. **Compute**: o ambiente passivo (VPC, EKS, RDS restaurado, NLB, observabilidade) normalmente não existe (_state_ vazio em `terra-dr/`). Ativá-lo significa restaurar o RDS a partir do backup replicado (`aws_db_instance.restored`) e provisionar o restante do zero.
...
- **A fase 2 (decisão de declarar desastre) não está automatizada de propósito.** Um failover automático de compute (sem revisão manual) poderia disparar uma restauração de RDS e um `terraform apply` completo em resposta a uma falha transitória, o que tem custo e risco maiores do que aguardar poucos minutos de confirmação.
