...
A detecção de incidentes é feita por regras de alerta que notificam o Grafana IRM. O assistente de IA do Grafana Cloud é usado principalmente na triagem e no diagnóstico de incidentes. O Grafana também oferece recursos nativos de detecção de anomalias e _forecasting_ (_Adaptive Traces_, _Outlier Detection_ e outros), resumidos no documento de incidentes. O ciclo de vida completo de incidentes, que inclui detecção, triagem, comunicação, diagnóstico, mitigação, _post-mortem_ e melhoria contínua, está disponível em [`doc/incidentes.md` ⤴️][incidentes].
